#!/usr/bin/env bash
# Build the runtime image and export it as a docker-archive tar the s9pk packer
# loads, with the exact `start9/<id>/main:<version>` RepoTag `start-sdk verify`
# requires.
#
...
#
# Usage: build-image-tar.sh <pkg_id> <version> <arch_tar_path>
set -euo pipefail
...
if command -v docker >/dev/null 2>&1; then
  echo "[build-image-tar] docker buildx -> $OUT"
...
echo "[build-image-tar] podman build $TAG"
