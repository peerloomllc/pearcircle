...
## Slice plan
- **Slice 1 - version visibility (foundation):** stamp the build version; expose it via `seeder:status` + `/api/status` + the localhost UI; report it over the seeder-admission channel; the mobile app shows each seeder's version. Independently valuable: operators can finally *see* which seeders are behind, which also serves the lastseen-ephemeral phase-2 convergence story (who's holding the gate open).
- **Slice 2 - update check + notification:** the host's GitHub-Releases poll + `/api/update`; the localhost "Update available" banner; the mobile app's "update available / out of date" flag. Notify-only at this point (the banner can deep-link to the release as a stopgap apply).
- **Slice 3 - one-click apply:** start with the no-privilege-helper platforms (Windows service, Linux AppImage), then the privileged-helper path for macOS `.pkg` + Linux `.deb`. Verified download -> stop -> swap -> restart, data dir preserved.

## Open questions
- **macOS / Linux-deb privilege.** Install a small privileged updater helper at first install (macOS `LaunchDaemon` running as root, or a `SMAppService`/`SMJobBless`-style helper; Linux a polkit action or a root-owned updater dropped by the `.deb` postinst) so subsequent updates apply one-click without re-prompting sudo - vs. prompting for elevation per update. Leaning toward the install-once helper for true seamlessness, but it widens the security surface (a root helper that fetches + runs installers) and needs careful scoping.
