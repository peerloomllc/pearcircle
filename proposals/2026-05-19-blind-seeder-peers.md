...
**Revocation-enforcement correction (same date).** Field-testing the global setup surfaced a slice-3d bug: the revocation peer-filter destroyed the whole Hyperswarm connection (`conn.destroy()`) when a seeder was revoked in any circle. Because one connection multiplexes every shared circle *and* the admission Protomux channel, that (a) collateral-killed replication for circles where the seeder was still validly admitted, and (b) permanently bricked re-admission — a revoked seeder's re-announce can't cross a destroyed connection, so the proposal's stated "re-admit by writing a fresh `seeder:` row" was impossible in practice. Fix: the peer-filter no longer destroys the connection. `src/lib/seederPeerFilter.js` now exposes `classifySeederConnection` returning `admitted` / `revoked-everywhere` / `none`; `onSwarmConnection` skips `corestore.replicate` only when the seeder is `revoked-everywhere` (revoked in every circle it has a row in, admitted in none) and otherwise keeps replicating. The connection and its admission channel always stay open, so a revoked seeder can re-announce and be re-admitted. The admission receiver's dedup was also corrected: a *revoked* existing `seeder:` row no longer suppresses the `seeder:announced` event — re-announce after revocation is a re-admission request and must reach the user's approval prompt. Consequence: revocation is enforced (no new blocks to a fully-revoked seeder) without bricking the connection; a seeder revoked in circle A but still admitted in circle B keeps replicating — it is blind, so circle A's blocks are ciphertext to it (a bandwidth cost only, no exposure). This also resolves the earlier-flagged per-circle-revoke / per-connection-enforcement collateral-damage quirk.

### 2026-05-20 — seeder auto-follow (one-time setup, new circles seed automatically)

**Tier: T2.** No replicated-record or invite-grammar change. One new Protomux protocol, additive IPC, local-only registry. See `DECISIONS.md` 2026-05-20.

**Motivation.** The global-mint amendment made setup one action, but the bundle is a *snapshot* — create a circle later and you must re-mint, re-paste, and re-approve. A seeder operator expects setup to be genuinely one-time: designate a device once, and every circle created or joined afterward seeds itself.

**Change.**

- **Followed-seeder registry.** A member marks a seeder device as "followed" via a per-seeder toggle in Settings → Seeders (default off; an explicit opt-in so a seeder admitted to a shared circle never silently starts following the user's other circles). Stored as local-only `seederfollow:{pubkey}` Hyperbee rows — never replicated. New IPC `circle:seeder:follow:set { pubkey, enabled }`; `seeders:listAll` gains a `followed: boolean` per entry.
- **New Protomux channel `pearcircle/seeder-sync/1`** (`src/seederSync.js`) — a device-level channel (single fixed id, not per-circle, unlike `seeder-admission`). On any member↔peer connection the member opens it in `member` role; it sends the member's full seed bundle (`{ invites: string[] }`) only when the remote is a followed seeder (checked at send time, so a channel to a plain member peer stays idle). The seed-mode worklet opens it in `seed` role and receives.
- **Seeder auto-enroll.** On receiving a bundle the seeder enrolls any circle it isn't already in, via the shared `enrollSeedInvite` primitive (extracted from the `seeder:enroll` IPC body so the manual paste and the auto-push take the identical path). Idempotent.
- **Seeder-side trust gate.** The seeder auto-enrolls a pushed bundle only when the pushing member's connection pubkey is the `inviter` of ≥1 circle the seeder is already enrolled in — a random peer cannot spam enrollments.
- **Member auto-approve.** When a followed seeder announces in a circle, the member writes the `seeder:{pubkey}` admission row directly (reusing the `circle:seeder:approve` logic) and emits `seeder:admitted` instead of surfacing the `seeder:announced` approval banner. Other members of that circle still see the row and still approve independently — transparency unchanged; only the follow-owner's device skips the prompt.
- **Push timing.** The member re-pushes the bundle over every open sync channel on `circle:create` / `circle:join` and when a follow toggle is flipped on, so a new circle (or a freshly-followed seeder) is picked up without waiting for a reconnect.

**Scope.** Add-only: auto-follow enrolls new circles but never auto-un-enrolls. Leaving a circle does not drop it from the seeder — that stays manual (`seeder:leave` / revoke). The first circle still uses the manual bundle ("Set up a seeder device"); auto-follow covers every circle after the device is marked followed.

**Approval prompt dropped (same date).** Field-testing showed the announce → approve → admission-row handshake was both a friction point and unreliable for circles created after the seeder connection already existed (the announce didn't consistently reach the member). Resolution: **the human approval prompt is removed.** A blind seeder carries no encryption key and cannot read circle content; the `seeder:{pubkey}` row stays visible to every member (transparency) and revoke remains one tap, so a consent gate added friction without payoff. This revises Q7 — the original "any-member admission with transparency, the prompt names the admitting member" becomes "auto-admit with transparency": (1) the `SeederApprovalBanner` UI and the `seeder:announced` event are removed; (2) `handleSeederAnnounce` auto-admits any announcing seeder (writes the `seeder:` row if the member is a writer) — this is first-contact for a manually-set-up seeder; (3) for a followed seeder, `circle:create` writes the `seeder:` rows directly via `admitFollowedSeedersToCircle` — the founder is the writer, so the seeder appears in every member's Seeders list immediately with no cross-device handshake. The `seeder:` record kind, the admission Protomux channel (still the first-contact pubkey-discovery path), the peer-filter, and `circle:seeder:revoke` are all unchanged. Net safeguards: a seeder is always visible in the per-circle list and revocable by any member; only the human gate is gone.

### 2026-06-10 — slice 3d completion: replicate every member's writer core, not just the founder's

**Tier: T2.** One new additive Protomux message on the existing `seeder-admission` channel. No replicated-record kind changes, no invite-grammar change, no encryption change. Backward compatible at the wire-protocol-v1 floor (old members omit the message, old seeders ignore it). See `DECISIONS.md` 2026-06-10.

**Motivation.** v1 of this proposal (the `mountSeederCircle` comment at `src/bare.js:3108-3110`) only ever guaranteed **bootstrap-core replication**: the seeder opens the circle's bootstrap core (the founder's writer input core) and relies on corestore's on-demand session opening for everything else. On-demand opening only fires for cores the seeder is *asked* for and already holds; it never proactively pulls a non-founder writer's core. The result, confirmed on the live Hudgins family circle 2026-06-10 (`tools/seeder-coverage.js`): of 6 member writer cores, the seeder held **only the founder's** (length 344) and **zero blocks** for the other five (216 / 446 / 245 / 181 / 1). A blind seeder that mirrors one member is not the always-on backstop this proposal promised — every other member's transitions and durable writes only propagate when that member and a recipient are online simultaneously, exactly the co-presence gap the proposal set out to close. This was observed in the field as a member whose live position kept updating (ephemeral broadcast, no replication gate) while their place-transition history froze for a day (durable writes on an unmirrored writer core).

**Design.** Mirror the existing `lastknownCores` admission message (proposal 2026-06-04 slice 2b) one-for-one for writer cores. That message already proves the whole pattern: a member hands the blind seeder a list of `{pubkey, coreKey}`, the seeder opens each core blind (no `encryptionKey`) and `download()`s it. Writer cores need the identical treatment.

- **New admission message `[3] writerCores`** (`src/seederAdmission.js`), member → seed, same shape as `lastknownCores`:
  ```
  { cores: [ { pubkey: hex64, coreKey: hex64 }, ... ] }   // coreKey = that member's Autobase writer-core public key
  ```
  Validated at the wire boundary by a `normalizeWriterCores` clone of `normalizeLastknownCores`: drops malformed entries, caps at 256. `circleId` is the trusted channel id, never carried in the body. `setupSeederAdmissionChannel` gains `onWriterCores` (seed role) and returns `sendWriterCores` (member role), exactly paralleling the lastknown plumbing.
- **Seeder side** (`src/bare.js`): `_seederCircles` entries gain a `writerCores: Map<pubkeyHex, core>` alongside the existing `lastknownCores` map; `handleSeederWriterCores` is a near-verbatim clone of `handleSeederLastknownCores` — open `ns.get({ key })` blind, `core.download({ start: 0, end: -1, linear: false })`, track for retention + cleanup. Wired into both admission-channel setup sites (member-conn at ~3175 and seed-mode-conn at ~3967).
- **Member side** (`src/bare.js`): `collectCircleWriters(circleId)` enumerates the circle's writer core keys from `base.activeWriters` (the founder's bootstrap core is included and de-duped seeder-side, harmless). `sendWriterCores` is pushed on the same three triggers as `lastknownCores`: admission-channel open (after the seeder is confirmed), when an `addWriter` op lands, and on the `resend` path (`circle:create` / `circle:join` / follow toggle).

**Retrofit / no recreation.** Writer-core keys are stable for the life of a circle (they only change on a full local repair). Existing circles need no recreation: once members run this build and reconnect to a followed seeder, they push their writer keys and the seeder backfills each core's full history from whoever holds it. Two conditions for a given member's history to fully land on the seeder: that member is on this build and connects to the seeder once, and the member (or a peer already holding their core) is online once while the seeder is connected. After that the seeder carries it permanently.

**Retention.** Writer cores download full history and are NOT covered by the `pruneOlderThan` TTL sweep in this slice. The sweep's block-time sidecar is keyed `seeder:blockTime:{circleId}:{seq}` and `clearBlock` only clears `entry.core` (the bootstrap), both of which assume one core per circle — recording writer-core blocks in that per-circle-seq namespace would make the sweep clear the wrong blocks. Holding full writer history is the desired default for a backstop anyway; per-core TTL retention (key block-times by core, teach the sweep which core to clear) is a clean follow-up, tracked separately. The seeder's full-download of writer cores mirrors the bootstrap-core treatment (`core.download({start:0,end:-1})`).

**Security.** No change to the trust model. The seeder remains blind: it stores and forwards encrypted blocks for writer cores it is told about, exactly as it already does for the bootstrap and lastknown cores, and cannot decrypt any of them. A malicious member can at most push junk 64-hex keys; the seeder opens empty cores that never receive blocks (bounded by the 256 cap), a trivial resource cost identical to the existing lastknown surface.

**Verify.** Unit: `normalizeWriterCores` validation + cap (mirror `tests/` lastknown coverage). Integration: a two-writer circle with a seeder ends with the seeder holding *both* writer cores. Field: re-run `tools/seeder-coverage.js` against the Hudgins invite and confirm the seeder goes from 1/6 to 6/6 complete. Manual smoke: the frozen-transition repro (a backgrounded member's place transition reaches a second device with the first device offline, via the seeder).

**Rollback.** Remove the `writerCores` message and handler. Purely additive, so rollback is clean; circles fall back to bootstrap-only replication (today's behavior). No persisted-state migration either way.
