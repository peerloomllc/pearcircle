...
- **Two-phase rollout.**
  ...
  - *Phase 2 (growth-stop):* **capability detection (decided 2026-06-04; mechanism refined in slice 3).** A peer's new-path support is read from the durable `lastknownCore:{pubkey}` view announce (a slice-2a peer announces a core; an old peer does not), rather than a fresh ephemeral-channel handshake message - simpler, attributable, reconnect-proof, and works for muted members. A writer auto-stops its autobase `lastSeen` write for a circle once all of that circle's current members have announced a core. Self-driving and per-circle (no manual fleet-wide flag, no release coordination); growth stops for each circle as its members converge. A keep-writing kill-switch stays for safety. This is gated on per-circle writer support because an old peer would otherwise stop seeing upgraded peers' positions.
...
- **Slice 3 (shipped):** capability detection + phase-2 cutover (stop the Autobase `lastSeen` write). **Design refinement (2026-06-04):** the "advertised support" signal is the **durable `lastknownCore:{pubkey}` view announce** (slice 2a) rather than a fresh ephemeral-channel handshake message - a writer that has announced a core is a slice-2a+ peer that both writes a core and reads peers' cores. This is simpler and more robust than a live-channel handshake (durable, attributable, survives reconnects, no protomux drift against slice-1/2 peers, works for muted members). `src/lib/lastSeenCutover.js` (`allMembersAnnouncedCore`, pure + unit-tested): cutover engages only when **every** currently-visible member has announced a core; any unsupported member (old peer, or not-yet-announced joiner) keeps the write on, so no peer silently stops seeing positions. In `src/bare.js`: recomputed off the hot path (5s sweep) into `_lastSeenCutoverCircles`; the `location:update` and `appendLastSeen` (geofence/region) write paths skip the Autobase write for cutover circles while the live broadcast + core append still run; the last written value stays in the view so a brand-new old joiner still sees a position. Keep-writing kill-switch (`config:forceAutobaseLastSeen`, IPC `lastSeen:cutover:get` / `lastSeen:cutover:setForceWrite`, restored on boot) forces the durable write back on everywhere. Marks: `lastseen:cutover` / `lastseen:cutover-reverted` / `lastseen:cutover-blocked {members, announced, missing}`. Verified: unit test (decision rule); `scripts/lastseen-cutover-smoke.js` (real worklet - engage on convergence, kill-switch revert + re-engage); and on-device (circle `2fwYeqiU`, all three members - iPhone + Pixel 9 + D2) BOTH directions: the safety property (`cutover-blocked {members:3, announced:2, missing:1}` held the write on while the iPhone was still on an old build) AND, after the iPhone was upgraded to this build and announced its core (`c04740b0`), the positive case - all three writers engaged `lastseen:cutover {members:3}` and stopped the Autobase write, so the oplog stopped growing for the circle.
